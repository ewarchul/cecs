                             ...
                             int r_flag) /* Bent_Cigar */
{
  ...
  cec2017_sr_func(x, z, nx, Os, Mr, 1.0, s_flag, r_flag,
                  y); /* shift and rotate */
...
void cec2017_sphere_func(double *x, double *f, int nx, double *Os, double *Mr,
  ...
  double *y = malloc(nx * sizeof(double));
  double *z = malloc(nx * sizeof(double));
  cec2017_sr_func(x, z, nx, Os, Mr, 1.0, s_flag, r_flag, y);
  for (i = 0; i < nx; i++) {
    f[0] += z[i] * z[i];
  }
  free(y);
...
void cec2017_rosenbrock_func(double *x, double *f, int nx, double *Os,
  ...
  double *y = malloc(nx * sizeof(double));
  double *z = malloc(nx * sizeof(double));
  cec2017_sr_func(x, z, nx, Os, Mr, 2.048 / 100.0, s_flag, r_flag, y);
...
void cec2017_weierstrass_func(double *x, double *f, int nx, double *Os,
  ...
  double *y = malloc(nx * sizeof(double));
  double *z = malloc(nx * sizeof(double));
  cec2017_sr_func(x, z, nx, Os, Mr, 0.5 / 100.0, s_flag, r_flag, y);
  for (i = 0; i < nx; i++) {
    sum = 0.0;
    sum2 = 0.0;
    for (j = 0; j <= k_max; j++) {
...
void cec2017_griewank_func(double *x, double *f, int nx, double *Os, double *Mr,
  ...
  double *y = malloc(nx * sizeof(double));
  double *z = malloc(nx * sizeof(double));
  cec2017_sr_func(x, z, nx, Os, Mr, 600.0 / 100.0, s_flag, r_flag, y);
  for (i = 0; i < nx; i++) {
    s += z[i] * z[i];
...
void cec2017_rastrigin_func(double *x, double *f, int nx, double *Os,
  ...
  double *y = malloc(nx * sizeof(double));
  double *z = malloc(nx * sizeof(double));
  cec2017_sr_func(x, z, nx, Os, Mr, 5.12 / 100.0, s_flag, r_flag, y);
  for (i = 0; i < nx; i++) {
...
void cec2017_schwefel_func(double *x, double *f, int nx, double *Os, double *Mr,
  ...
  cec2017_sr_func(x, z, nx, Os, Mr, 1000.0 / 100.0, s_flag, r_flag, y);
...
void cec2017_katsuura_func(double *x, double *f, int nx, double *Os, double *Mr,
  ...
  double *y = malloc(nx * sizeof(double));
  double *z = malloc(nx * sizeof(double));
  cec2017_sr_func(x, z, nx, Os, Mr, 5.0 / 100.0, s_flag, r_flag, y);
...
void cec2017_happycat_func(double *x, double *f, int nx, double *Os, double *Mr,
                           int s_flag, int r_flag) {
  int i;
...
void cec2017_hgbat_func(double *x, double *f, int nx, double *Os, double *Mr,
                        int s_flag, int r_flag) {
  int i;
  double alpha, r2, sum_z;
  alpha = 1.0 / 4.0;
