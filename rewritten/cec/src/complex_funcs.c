                  ...
                  double *bias, int r_flag) /* Composition Function 3 */
{
  ...
  i = 3;
  weierstrass_func(x, &fit[i], nx, &Os[i * nx], &Mr[i * nx * nx], 1,
                           r_flag);
                  ...
                  double *bias, int r_flag) /* Composition Function 4 */
{
  ...
  fit[i] = fit[i] * 10.0;
  i = 4;
  happycat_func(x, &fit[i], nx, &Os[i * nx], &Mr[i * nx * nx], 1,
                        r_flag);
  fit[i] = 10000.0 * fit[i] / 1.0e+3;
  cf_cal(x, f, nx, Os, delta, bias, fit, cf_num);
}
                  ...
                  double *bias, int r_flag) /* Composition Function 6 */
{
  ...
  fit[i] = 10000.0 * fit[i] / 1.0e+3;
  i = 1;
  grie_rosen_func(x, &fit[i], nx, &Os[i * nx], &Mr[i * nx * nx], 1,
                          r_flag);
  fit[i] = 10000.0 * fit[i] / 4.0e+3;
  i = 2;
  schwefel_func(x, &fit[i], nx, &Os[i * nx], &Mr[i * nx * nx], 1,
                  ...
                  double *bias, int r_flag) /* Composition Function 7 */
{
  ...
  i = 6;
  katsuura_func(x, &fit[i], nx, &Os[i * nx], &Mr[i * nx * nx], 1,
                        r_flag);
