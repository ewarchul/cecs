...
cec <- function(func_index, x, cec, max_func_index, dims, suite = NULL) {
  ...
  if (base::is.numeric(func_index) && func_index >= 1 && func_index <= max_func_index) {
    if (base::is.vector(x)) {
      row <- 1
      col <- base::length(x)
    ...
    base::stop(stringr::str_interp("Invalid argument: 'func_index' should be an integer between 1 and ${max_func_index} !"))
  }
  return(f)
}
